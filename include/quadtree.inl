...
inline bool qt::QuadTree<T>::insert(T *p) {
  ...
  if (!boundary.containsPoint(*p)) {
    ...
    if (!is_divided) {
      ...
      ret = (nw->insert(p) || ne->insert(p) || sw->insert(p) || se->insert(p));
    }
  }
...
inline void qt::QuadTree<T>::show(sf::RenderWindow *window, sf::Color color) {
  if (is_divided) {
    ...
    node.setSize(sf::Vector2f(boundary.w * 2, boundary.h * 2));

    node.setPosition(boundary.x - boundary.w, boundary.y - boundary.h);
